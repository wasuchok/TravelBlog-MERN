import { Box, Flex, HStack, Link, IconButton, Icon, Text, useDisclosure, Stack, useColorMode, useColorModeValue as mode, MenuItem, Menu, MenuList, MenuButton, ButtonGroup, useColorModeValue } from '@chakra-ui/react'
...
const NavLink = ({ path, children }) => (
  <Link as={ ReactLink } to={ path } px='2' py='2' fontWeight='semibold' _hover={{ textDecoration : 'none', bg: mode('blue.100', 'blue.800')}}>
    {children}
  </Link>
)
  ...
  return (
    ...
    <Box bg={mode('blue.200', 'blue.900')} px={4}>
      <Flex h='16' alignItems="center" justifyContent="space-between">
        <IconButton bg={mode('blue.200', 'blue.900')} icon={isOpen ? <CloseIcon /> : <HamburgerIcon />} display={{md: 'none'}} 
    ...
    <HStack>
      ...
      <HStack display={{base : 'none', md: 'flex'}} pl='10'>
        ...
        <Menu>
          <MenuButton fontWeight='semibold' p="2"  _hover={{ textDecoration : 'none', bg: mode('blue.100', 'blue.800')}}>
            Blog
          </MenuButton>
          <MenuList>
            {blogLinks.map((link) => (
   ...
   {isOpen ? (
      <Box pb='4' display={{md : 'none'}}>
        ...
        <Menu>
          <MenuButton textAlign='left' w='full' mt='3' fontWeight='semibold' p="2"  _hover={{ textDecoration : 'none', bg: mode('blue.100', 'blue.800')}}>
            Blog
          </MenuButton>
          <MenuList>
            {blogLinks.map((link) => (
