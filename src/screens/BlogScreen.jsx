...
const BlogScreen = () => {
    ...
    return (
        <VStack spacing='30px' minHeight='100vh'>
            ...
            { loading  
            ? <Stack direction='row' spacing='4'><Spinner mt='20' thickness='2px' speed='0.65s' emptyColor='gray.200' color='blue.500' size='xl' /></Stack>
            : error ? <Alert status='error'><AlertIcon /><AlertTitle>We are sorry!</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>
            : 
            <>
                ...
                {blogPosts.map((post) => (
                    <Box key={post._id} maxW={{base : '3xl', lg: '7xl'}} px={{base : '6', md : '8', lg : '20'}} py='6'>
                        <Stack direction={{base : 'column', lg : 'row'}} spacing='7'>
                            <Image src={post.image} min={{lg : '400px'}} maxH='280px' loading={<Spinner />} fit='cover' />
