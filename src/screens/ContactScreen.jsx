...
useEffect(() => {
    const credentials = async () => {
        const { data } = await axios.get(`${import.meta.env.VITE_API}/config/emailjs`)
        setEmailjsInfo(data)
    }
    credentials()
},[emailjs])
  ...
  const sendEmail = (values, reset) => 
    emailjs.send(emailjsInfo.service_id, emailjsInfo.template_id, values, emailjsInfo.public_key)
    .then((response) => {
        {toast({title: 'Email sent successfully', description: 'Thanks for sending.', status : 'success', duration : 7000, isClosable : true})}
        reset()
        setTimeout(() => {
        ...
        toast({
            title : 'Whoops!',
            description : 'Something went weong. Please try again later.',
            status : 'error',
            duration : 7000,
            isClosable : true
        })
